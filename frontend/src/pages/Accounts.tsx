...
function getCompactStatusMeta(acc: any) {
  ...
  const parts = [
    `Vòng đời:${getLifecycleStatus(acc)}`,
    `Gói:${getPlanState(acc)}`,
    `Hiệu lực:${getValidityStatus(acc)}`,
  ]
  const remainingCredits = overview?.remaining_credits
  const usageTotal = overview?.usage_total
  if (remainingCredits || usageTotal) {
    parts.push(`Hạn mức:${remainingCredits || '-'} / Đã dùng:${usageTotal || '-'}`)
  }
  return parts.join(' / ')
}
...
function RegisterModal({
  ...
  // chatgpt 平台特定：注册成功后是否自动获取支付链接（Lưu到账号 cashier_url 字段，
  // 后续点"打开支付链接"直接复用）。仅当 platform === 'chatgpt' 时显示开关。
  const [autoPaymentLink, setAutoPaymentLink] = useState(false)
  // GoPay 专属：PIN (6 chữ số)、Hero-SMS API key、注册代理。仅当
  ...
  // Hero-SMS getNumber 价格Giới hạn（USD，小数）。0.011 ≈ 175 IDR，对 GoPay
  ...
  const start = async () => {
    ...
    try {
      ...
      if (platform === 'gopay') {
        if (!gopayApiKey.trim()) {
          throw new Error('Đăng ký GoPay bắt buộc nhập Hero-SMS API key')
        }
        if (!/^\d{6}$/.test(gopayPin.trim())) {
          throw new Error('GoPay PIN phải gồm 6 chữ số')
        }
        extra.herosms_api_key = gopayApiKey.trim()
        extra.gopay_pin = gopayPin.trim()
        if (gopayProxy.trim()) extra.gopay_proxy = gopayProxy.trim()
        // maxPrice 走 Hero-SMS getNumber 参数，Đơn vị USD，0/空表示不限
          ...
          {!taskId ? (
            configLoading ? (
              ...
              <>
                ...
                {/* GoPay-specific SMS number + PIN + proxy settings (platform === 'gopay') */}
                {platform === 'gopay' && (
                  <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3 space-y-3">
                    <div className="text-sm font-medium text-[var(--text-primary)]">Tham số đăng ký GoPay</div>
                    <div>
                      <label className="text-xs text-[var(--text-muted)] block mb-1">Hero-SMS API key（bắt buộc）</label>
                      <input
                        type="text"
                        value={gopayApiKey}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs text-[var(--text-muted)] block mb-1">PIN (6 chữ số)</label>
                        <input
                          type="text"
                          maxLength={6}
                          value={gopayPin}
                      ...
                      <div>
                        <label className="text-xs text-[var(--text-muted)] block mb-1">Giới hạn giá nhận SMS (USD)</label>
                        <input
                          type="text"
                          value={gopayMaxPrice}
                    ...
                    <div>
                      <label className="text-xs text-[var(--text-muted)] block mb-1">Proxy đăng ký (tùy chọn)</label>
                      <input
                        type="text"
                        value={gopayProxy}
                    ...
                    <div className="text-xs text-[var(--text-muted)]">
                      Để trống thì lần lượt dùng biến môi trường OPAI_HEROSMS_API_KEY / OPAI_GOPAY_DEFAULT_PIN / OPAI_GOPAY_REGISTER_PROXY / OPAI_HEROSMS_MAX_PRICE_USD. Đặt maxPrice = 0 để không giới hạn giá.
                    </div>
                  </div>
                )}

                {/* ChatGPT-specific: automatically fetch payment link after registration and save cashier_url to account extra */}
                {platform === 'chatgpt' && (
                  <label className="flex items-start gap-2 rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3 cursor-pointer hover:border-[var(--accent)]/60">
                    ...
                    <div className="flex-1 text-xs text-[var(--text-secondary)]">
                      <div className="text-sm font-medium text-[var(--text-primary)]">
                        Tự động lấy liên kết thanh toán sau khi đăng ký thành công
                      </div>
                      <div className="mt-0.5">
                        Tạo liên kết thanh toán Plus (không checkout tự động) và lưu vào tài khoản; sau này bấm “Mở liên kết thanh toán” để dùng lại.
                      </div>
                    </div>
                  </label>
                )}
                ...
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3 text-xs text-[var(--text-secondary)]">
                  ...
                  {selection.identityProvider === 'oauth_browser' && !reusableBrowser && (
                    <div className="mt-2 text-amber-400">Tự động trình duyệt nền phụ thuộc Chrome Profile hoặc Chrome CDP; nếu chưa cấu hình thì chỉ cho phép trình duyệt hiển thị.</div>
                  )}
                </div>
  ...
  return (
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <h2 className="text-base font-semibold text-[var(--text-primary)]">Thêm tài khoản thủ công</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]"><X className="h-4 w-4" /></button>
        </div>
        <div className="px-6 py-4 space-y-3">
          {[['email','Email','text'],['password','Mật khẩu','text'],['primary_token','Thông tin xác thực chính','text'],['cashier_url','Liên kết trial','text']].map(([k,l,t]) => (
            <div key={k}>
          ...
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">Trạng thái vòng đời</label>
            <select value={form.lifecycle_status} onChange={e => set('lifecycle_status', e.target.value)}
              ...
              <option value="subscribed">Đã đăng ký gói</option>
            </select>
          </div>
        </div>
    ...
    <div className="space-y-3">
      {sections.map((section: any) => (
        <div key={section?.key || section?.title} className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] p-3">
          <div className="text-xs font-semibold text-[var(--text-primary)]">{section?.title || 'Chi tiết'}</div>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
...
function ActionResultHighlights({ payload }: { payload: any }) {
  ...
  if ('valid' in payload) stats.push({ label: 'Tài khoản hợp lệ', value: payload.valid })
  if (payload.membership_type) stats.push({ label: 'Gói', value: payload.membership_type })
  if (payload.plan) stats.push({ label: 'Gói', value: payload.plan })
  if (payload.plan_id) stats.push({ label: 'Plan ID', value: payload.plan_id })
  if (typeof payload.has_valid_payment_method === 'boolean') stats.push({ label: 'Đã gắn thẻ', value: payload.has_valid_payment_method })
  if ('trial_eligible' in payload) stats.push({ label: 'Có thể trial', value: payload.trial_eligible })
  if (payload.trial_length_days) stats.push({ label: 'Số ngày trial', value: payload.trial_length_days })
  if (payload.remaining_credits) stats.push({ label: 'Còn lạiHạn mức', value: payload.remaining_credits })
  if (payload.usage_total) stats.push({ label: 'Đã dùngHạn mức', value: payload.usage_total })
  if (payload.plan_credits) stats.push({ label: 'Tổng hạn mức', value: payload.plan_credits })
  if (payload.usage_summary?.plan_title) stats.push({ label: 'Kiro Gói', value: payload.usage_summary.plan_title })
  if ('days_until_reset' in (payload.usage_summary || {})) stats.push({ label: 'Đếm ngược reset', value: payload.usage_summary?.days_until_reset })
  if (payload.usage_summary?.next_reset_at) stats.push({ label: 'Lần reset kế tiếp', value: payload.usage_summary.next_reset_at })
  if ('available' in (payload.portal_session || {})) stats.push({ label: 'Portal khả dụng', value: payload.portal_session?.available })
  if (payload.desktop_app_state?.app_name) stats.push({ label: 'Ứng dụng desktop', value: payload.desktop_app_state?.app_name })
  if ('running' in (payload.desktop_app_state || {})) stats.push({ label: 'Desktop đã mở', value: payload.desktop_app_state?.running })
  if ('ready' in (payload.desktop_app_state || {})) stats.push({ label: 'Desktop sẵn sàng', value: payload.desktop_app_state?.ready })
  if (payload.key_prefix) stats.push({ label: 'Tiền tố API key', value: payload.key_prefix })
  if (payload.key_prefix && payload.name) stats.push({ label: 'Tên key', value: payload.name })
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            {cursorModels.map(([model, info]: [string, any]) => (
              <div key={model} className="rounded-lg border border-[var(--border)] bg-black/20 p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-[var(--text-secondary)]">
                  <div>Số request: {formatResultValue(info?.num_requests)}</div>
                  <div>Tổng request: {formatResultValue(info?.num_requests_total)}</div>
                  <div>Token: {formatResultValue(info?.num_tokens)}</div>
                  <div>Request còn lại: {formatResultValue(info?.remaining_requests)}</div>
                  <div>Giới hạn request: {formatResultValue(info?.max_request_usage)}</div>
                  <div>Giới hạn token: {formatResultValue(info?.max_token_usage)}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            {kiroBreakdowns.map((item: any, index: number) => (
              <div key={`${item.resource_type || item.display_name}-${index}`} className="rounded-lg border border-[var(--border)] bg-black/20 p-3">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-[var(--text-secondary)]">
                  <div>Đã dùng: {formatResultValue(item.current_usage)}</div>
                  <div>Giới hạn: {formatResultValue(item.usage_limit)}</div>
                  <div>Còn lại: {formatResultValue(item.remaining_usage)}</div>
                  <div>Đơn vị: {formatResultValue(item.unit)}</div>
                  <div>Trạng thái trial: {formatResultValue(item.trial_status)}</div>
                  <div>Trial hết hạn: {formatResultValue(item.trial_expiry)}</div>
                  <div>Giới hạn trial: {formatResultValue(item.trial_usage_limit)}</div>
                  <div>Trial còn lại: {formatResultValue(item.trial_remaining_usage)}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <div>
            ...
            <p className="text-xs text-[var(--text-muted)] mt-0.5">Kết quả thao tác</p>
          </div>
        ...
        <div className="relative overflow-hidden border-b border-[var(--border)] px-6 py-5">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-[var(--text-muted)]">Hiển thị tập trung trạng thái tác vụ, tóm tắt lỗi và log thời gian thực</p>
            </div>
            <div className="flex items-center gap-2">
              {taskStatus ? (
  ...
  return (
    <div className="dialog-backdrop" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)]">
          <div>
            <h2 className="text-base font-semibold text-[var(--text-primary)]">{action?.label || 'Tham số thao tác'}</h2>
            <p className="text-xs text-[var(--text-muted)] mt-0.5">Điền tham số cần thiết để thực hiện thao tác này</p>
          </div>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-primary)]">
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="px-6 py-4 border-t border-[var(--border)] flex gap-3">
          <Button onClick={() => onSubmit(form)} disabled={submitting} className="flex-1">
            {submitting ? 'Đang thực thi...' : 'Thực thi'}
          </Button>
          <Button variant="outline" onClick={onClose} disabled={submitting} className="flex-1">Hủy</Button>
        </div>
      </div>
    </div>
  )
...
function ActionMenu({
  ...
  const handleActionDone = async (status: string) => {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        if (actionUrl) {
          setToast({ type: 'success', text: data.message || 'Đã mở liên kết thanh toán trong tab mới và sao chép liên kết' })
          return
        }
        ...
        setToast({ type: 'success', text: data.message || 'Thao tác thành công' })
        return
      }
      setToast({ type: 'success', text: typeof data === 'string' && data ? data : 'Thao tác thành công' })
    } catch (error: any) {
      setToast({ type: 'error', text: error?.message || 'Đọc kết quả tác vụ thất bại' })
    }
  }
  ...
  return (
    <div className="relative flex min-w-[136px] items-center justify-end gap-1.5 whitespace-nowrap">
      ...
      <button onClick={onDetail} className="table-action-btn">Chi tiết</button>
      {actions.length > 0 && (
        <div className="relative">
          <button ref={triggerRef} onClick={() => setOpen(o => !o)}
            className="table-action-btn">Thêm ▾</button>
            ...
            >
              {actions.map(a => (
                <button key={a.id}
                  ...
                  {running === a.id ? 'Đang thực thi...' : a.label}
                </button>
              ))}
              ...
              <button
                onClick={() => {
                  setOpen(false)
                  if (confirm(`Xác nhận xóa ${acc.email}？`)) {
      ...
      {actions.length === 0 && (
        <button
          onClick={() => { if (confirm(`Xác nhận xóa ${acc.email}？`)) apiFetch(`/accounts/${acc.id}`, { method: 'DELETE' }).then(onDelete) }}
    ...
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm flex flex-col" style={{maxHeight:'90vh'}} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border)] shrink-0">
          <div>
            <h2 className="text-base font-semibold text-[var(--text-primary)]">Chi tiết tài khoản</h2>
        ...
        <div className="px-6 py-4 space-y-3 flex-1 overflow-y-auto min-h-0">
          <div className="relative overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--accent-soft)] p-4 shadow-[var(--shadow-soft)]">
            ...
            <div className="relative flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-xs uppercase tracking-[0.18em] text-[var(--text-muted)]">Trạng thái chính</div>
              ...
              <div className="grid grid-cols-2 gap-2 text-right text-[11px] text-[var(--text-muted)] sm:grid-cols-3">
                <div className="rounded-xl border border-[var(--border-soft)] bg-black/10 px-2.5 py-2">
                  <div className="uppercase tracking-[0.12em]">Vòng đời</div>
                  <div className="mt-1 text-[var(--text-primary)]">{getLifecycleStatus(acc)}</div>
                </div>
                <div className="rounded-xl border border-[var(--border-soft)] bg-black/10 px-2.5 py-2">
                  <div className="uppercase tracking-[0.12em]">Hiệu lực</div>
                  <div className="mt-1 text-[var(--text-primary)]">{getValidityStatus(acc)}</div>
                </div>
                <div className="rounded-xl border border-[var(--border-soft)] bg-black/10 px-2.5 py-2">
                  <div className="uppercase tracking-[0.12em]">Trạng thái gói</div>
            ...
            <div className="space-y-2">
              ...
              {verificationMailbox?.email && (
                <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] px-3 py-2 text-xs text-[var(--text-secondary)]">
                  Email xác minh: {verificationMailbox.email} · {verificationMailbox.provider || '-'} · ID {verificationMailbox.account_id || '-'}
                </div>
              )}
            </div>
          )}
              ...
              {providerAccounts.map((item: any, index: number) => (
                <div key={`${item.provider_name || 'provider'}-${item.login_identifier || index}`} className="rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] p-3">
                  ...
                  <div className="mt-1 text-xs text-[var(--text-secondary)] break-all">
                    Định danh đăng nhập: {item.login_identifier || '-'}
                  </div>
          ...
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">Thông tin xác thực chính</label>
          ...
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">Liên kết trial</label>
...
function ImportModal({ platform, onClose, onDone }: { platform: string; onClose: () => void; onDone: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setResult(`Nhập thành công ${res.created} tài khoản`); onDone()
    } catch (e: any) { setResult(`Thất bại: ${e.message}`) } finally { setLoading(false) }
  }
  return (
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-base font-semibold text-[var(--text-primary)] mb-2">Nhập hàng loạt</h2>
        <p className="text-xs text-[var(--text-muted)] mb-3">Định dạng mỗi dòng: <code className="bg-[var(--bg-hover)] px-1 rounded">email password [cashier_url]</code></p>
        ...
        <div className="flex gap-2">
          <Button onClick={submit} disabled={loading} className="flex-1">{loading ? 'Đang nhập...' : 'Nhập'}</Button>
          <Button variant="outline" onClick={onClose} className="flex-1">Hủy</Button>
        </div>
      </div>
    </div>
...
function ExportMenu({
  ...
  const doExport = async (format: string) => {
    ...
    try {
      ...
      window.alert(e?.message || 'Xuất thất bại')
    } finally {
      setLoading(null)
    }
  }
  ...
  const options = [
    ...
    { key: 'any2api', label: 'Xuất Any2Api' },
    { key: 'sub2api', label: 'Xuất Sub2Api' },
    { key: 'cpa', label: 'Xuất CPA' },
    ...(platform === 'kiro' ? [{ key: 'kiro-go', label: 'Xuất Kiro-Go' }] : []),
  ]
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        {loading ? 'Đang xuất...' : hasSelection ? `Xuất mục đã chọn(${selectedIds.length})` : 'Xuất'}
      </Button>
      {open && (
        <div className="absolute right-0 top-10 z-20 min-w-[148px] rounded-lg border border-[var(--border)] bg-[var(--bg-card)] py-1 shadow-lg">
          <div className="px-3 py-1 text-[11px] text-[var(--text-muted)]">
            {hasSelection ? `Xuất ${selectedIds.length} tài khoản đã chọn` : 'Xuất kết quả lọc hiện tại'}
          </div>
          {options.map(option => (
            <button
              key={option.key}
              ...
              <tr key={acc.id} className="group border-b border-[var(--border)]/30 hover:bg-[var(--text-primary)]/[0.02] transition-colors cursor-pointer"
                ...
                <td className="px-3 py-2.5 font-mono text-sm text-[var(--text-primary)] align-top">
                  <div className="flex min-w-0 items-center gap-1.5">
                    ...
                    <button onClick={e => { e.stopPropagation(); copy(emailApiLine(acc.email)) }} title="Sao chép Email + API email" className="text-[var(--text-muted)] hover:text-[var(--text-primary)] opacity-0 group-hover:opacity-100 transition-opacity"><Copy className="h-3 w-3" /></button>
                  </div>
                  {verificationMailbox && (verificationMailbox.email || verificationMailbox.account_id || verificationMailbox.provider) && (
                    <div
                      ...
                      title={`Email xác minh: ${verificationMailbox.email || '-'} · ${verificationMailbox.provider || '-'}`}
                    >
                  ...
                  {overview?.remote_email && overview.remote_email !== acc.email && (
                    <div className="mt-1 truncate text-xs text-[var(--text-muted)]" title={`Email từ xa: ${overview.remote_email}`}>
                      Email từ xa: {overview.remote_email}
                    </div>
                  )}
                  {displayBadges.length > 0 && (
                ...
                <td className="px-3 py-2.5 align-top">
                  {getCashierUrl(acc) ? (
                    <div className="flex items-center gap-1.5 whitespace-nowrap opacity-70 group-hover:opacity-100 transition-opacity">
                      <button onClick={e => { e.stopPropagation(); copy(getCashierUrl(acc)) }} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors p-0.5 rounded hover:bg-[var(--bg-pane)]" title="Sao chép liên kết"><Copy className="h-3 w-3" /></button>
                      <a href={getCashierUrl(acc)} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors p-0.5 rounded hover:bg-[var(--bg-pane)]" title="Mở trang thanh toán"><ExternalLink className="h-3 w-3" /></a>
                    </div>
